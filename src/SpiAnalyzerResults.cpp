    ...
    if ((frame.mFlags & SPI_ERROR_FLAG) == 0) {
        if (channel == mSettings->mMosiChannel) {
            ...
            if (frame.mType == 0) {
                ...
                snprintf(result, sizeof(result), "Start bit %s", number_str);
                AddResultString(result);
            }
            else if (frame.mType == 1) {
                AnalyzerHelpers::GetNumberString(frame.mData1, display_base, 2, number_str, 128);
                AddResultString(number_str);
                snprintf(result, sizeof(result), "Op Code %s", number_str);
                AddResultString(result);
            }
            else if (frame.mType == 2) {
                AnalyzerHelpers::GetNumberString(frame.mData1, display_base, 16, number_str, 128);
                snprintf(result, sizeof(result), "Addr %s", number_str);
                AddResultString(result);
            }
            else {
...
void SpiAnalyzerResults::GenerateExportFile(const char *file, DisplayBase display_base, U32 /*export_type_user_id*/)
{
    ...
    for (U32 i = 0; i < num_frames; i++) {
        ...
        if (mosi_used == true) {
            if (frame.mType == 0) {
                AnalyzerHelpers::GetNumberString(frame.mData1, display_base, 1, mosi_str, 128);
            }
            else if (frame.mType == 1) {
                AnalyzerHelpers::GetNumberString(frame.mData1, display_base, 2, mosi_str, 128);
            }
            else if (frame.mType == 2) {
                AnalyzerHelpers::GetNumberString(frame.mData1, display_base, 16, mosi_str, 128);
            }
            else {
