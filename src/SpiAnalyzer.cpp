...
void SpiAnalyzer::GetWord()
{
    ...
    U32 bits_per_step = 1;
    U32 step_count = 0;
    U64 mClockRaisingSample = 0;
    U64 mClockFallingSample = 0;
    ...
    while(1) {
        ...
        U64 mosi_word = 0;
        mosi_result.Reset(&mosi_word, mSettings->mShiftOrder, bits_per_step);
        DataBuilder miso_result;
        U64 miso_word = 0;
        miso_result.Reset(&miso_word, mSettings->mShiftOrder, bits_per_transfer);
        for (U32 i = 0; i < bits_per_step; i++) {
            ...
            mClockRaisingSample = mClock->GetSampleNumber();
            if (i == 0) {
            ...
            if ((i == (bits_per_step - 1)) && (mSettings->mDataValidEdge != AnalyzerEnums::Level)) {
            ...
            mClock->AdvanceToNextEdge();
            mClockFallingSample = mClock->GetSampleNumber();
            if (mSettings->mDataValidEdge == AnalyzerEnums::Level) {
                if (mMosi != NULL) {
                    mMosi->AdvanceToAbsPosition(mClockRaisingSample/2 + mClockFallingSample/2);
                    mosi_result.AddBit(mMosi->GetBitState());
                }
                if (mMiso != NULL && step_count >= 3) {
                    mMiso->AdvanceToAbsPosition(mClockRaisingSample/2 + mClockFallingSample/2);
                    miso_result.AddBit(mMiso->GetBitState());
                }
            }
